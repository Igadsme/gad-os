...
export async function POST(request: Request) {
  ...
  if (resendKey) {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: process.env.CONTACT_FROM_EMAIL ?? "Gad OS <noreply@imanigad.com>",
        to,
        reply_to: payload.email,
        subject: `[Gad OS] ${payload.subject} — ${payload.name}`,
        text: [
          payload.message,
          "",
    ...
    console.info("Contact message (no RESEND_API_KEY configured)", payload);
  }
